...
**E-learning · Quiz & Certification · Recommandation IA · Marketplace Étudiante**
...
| **Type** | Application Web E-learning (Projet Intégré Ingénieur) |
| **Framework** | Symfony 6.4 + PHP 8.2 |
| **Base de données** | MySQL / MariaDB 10.4 (partagée avec l'application JavaFX) |
...
> UniLearn est une plateforme e-learning nouvelle génération qui combine apprentissage en ligne, évaluation automatisée, certification PDF, assistance IA et marketplace étudiante — le tout dans une architecture multi-plateforme partageant une base de données commune entre une application web Symfony et un client desktop JavaFX.
...
| Analyse de Documents | Google Document AI | ✅ |
...
│  │           MySQL / MariaDB 10.4                              │  │
│  │     ┌─────────────────────────────────────────┐            │  │
│  │     │  Shared DB : Web Symfony ↔ JavaFX App    │            │  │
│  │     └─────────────────────────────────────────┘            │  │
│  └────────────────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────────────┘
```
...
| Google Document AI | Google Cloud | Analyse et extraction de documents |
...
**Problème** : Certains enregistrements `Certificate` avaient `quiz_result_id = NULL`, causant des erreurs Twig lors de l'accès à `certificate.quizResult.percentage`.

**Solution** : Vérifications null-safe dans les templates Twig avec l'opérateur ternaire : `certificate.quizResult is not null ? certificate.quizResult.percentage : 0`.
